...
const ManageClasses = () => {
  ...
  const handleClassApprove = (item) => {
    axiosSecure.patch(`/classes/approve/${item._id}`).then((res) => {
  ...
  const onSubmitUpdate = (data) => {
    ...
    axiosSecure
      .patch(`/classes/feedback/${updateItem?._id}`, data)
      .then((res) => {
        if (res.data.modifiedCount) {
          refetch();
  ...
  return (
    <>
            ...
            <tbody className="">
              ...
              {classes.map((item, index) => (
                <tr key={item?._id}>
                  ...
                  <th className="p-0">
                    {" "}
                    <span className="text-xs">{item?.TotalEnrolled}</span>
                  </th>
                  <td>
                    <div className="join join-vertical lg:join-horizontal">
                      ...
                      {item.status === "approve" && (
                        <button className="mx-0 btn join-item btn-xs btn-success">
                          approve
                        </button>
                      )}
                      {item.status === "denied" && (
                        <button className="mx-0 btn join-item btn-xs btn-error">
                          denied
                        </button>
                      )}
                    </div>
                  ...
                  <td>
                    <div className="join mx-0 join-vertical lg:join-horizontal">
                      <button
                        ...
                        className="btn join-item btn-xs btn-success text-xs"
                        disabled={
                          item.status === "approve" || item.status === "denied"
                        }
                      >
                        approve
                      </button>
                      <button
                        onClick={() => handleClassDenied(item)}
                        className="btn join-item btn-xs btn-error text-xs"
                        disabled={
                          item.status === "approve" || item.status === "denied"
                        }
                      >
                      ...
                      <button
                        ...
                        className="btn join-item btn-xs btn-warning"
                      >
                        send feedback
                      </button>
                    </div>
      ...
      <dialog
        id="my_modal_4"
        className={`modal ${isModalOpen ? "modal-open" : ""} modal-middle`}
      >
        <form onSubmit={handleSubmit(onSubmitUpdate)}>
          ...
          <div className=" text-left mx-3 my-3">
            <button type="submit" className="btn btn-secondary md:w-1/4 w-1/2">
              Send
            </button>
          </div>
        </form>
